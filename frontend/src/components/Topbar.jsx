...
export default function Topbar() {
  const {user,setUser,userFunction} = useContext(DataContext);
    return(
        <div className="bg-black p-4 border-x-8 border-y-8 border-white justify-between flex items-center fixed top-0 left-0 right-0">
              ...
              <Menu.Item>
                ...
                <Menu.Item>
                  {({ active }) => (
                    <button
                      // type="submit"
                      onClick={() => userFunction.remove(setUser)}
                      className={classNames(
                        active ? 'bg-gray-100 text-gray-900' : 'text-gray-700',
                        'block w-full px-4 py-2 text-left text-sm btn btn-danger'
                      )}
                    >
                      Sign out
                    </button>
